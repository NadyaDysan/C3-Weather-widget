class Suggest {
  constructor(element, options) {
    if (!(element instanceof HTMLElement)) {
      throw new Error("Non-HTML element was given");
    }
  ...
  onInput() {
    ...
    const filteredData = this.suggestData.filter((item) =>
      item.toLowerCase().startsWith(this.element.value)
    );
  ...
  renderSuggestPopup() {
    this.suggest = templateEngine(Suggest.suggestPopupTemplate(true));
  ...
  renderSuggestItems(data) {
    this.clearSuggest();
    const suggestItems = templateEngine(Suggest.suggestItemsTemplate(data));
...
Suggest.suggestPopupTemplate = (hidden) => ({
  tag: "div",
  cls: [
    "suggest__suggest-popup",
    hidden ? "suggest__suggest-popup_hidden" : undefined,
  ],
});
